...
TEST(UnitQuatf, Construction_FromAngleAxis)
{
...
TEST(UnitQuatf, Multiply_VectorRotation)
{
    const float angle = std::numbers::pi_v<float> / 1.234f;
    const Vec3f axis = Vec3f(1, 2, 3).Normalize();
    const UnitQuatf q(Radiansf(angle), axis);
...
TEST(UnitQuatf, CompoundMultiplication)
{
    const float angle = std::numbers::pi_v<float> / 2.0f;
    const Vec3f axis(0.0f, 0.0f, 1.0f);
...
TEST(UnitQuatf, Lerp)
{
    ...
    const UnitQuatf r1 = qA.Lerp(qB, 1.0f);
    ...
    const UnitQuatf r05 = qA.Lerp(qB, 0.5f);
...
TEST(UnitQuatf, InverseRotateVector)
{
    const float angle = std::numbers::pi_v<float> / 2.0f;
    const Vec3f axis(0.0f, 0.0f, 1.0f);
    const UnitQuatf q(Radiansf(angle), axis);
