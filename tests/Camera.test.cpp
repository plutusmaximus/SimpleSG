...
void ExpectMatrixNear(const Mat44f& actual, const Mat44f& expected)
{
...
void ExpectViewportEq(const Viewport& viewport,
    const uint32_t x,
    const uint32_t y,
...
float SignedDistanceToPlane(const Vec4f& plane, const Vec3f& point)
{
    return plane.Dot(Vec4f(point, 1.0f));
}

Vec3f PlaneNormal(const Vec4f& plane)
{
    return Vec3f(plane.x, plane.y, plane.z);
}

Vec3f MoveToSignedDistanceFromPlane(const Vec4f& plane, const Vec3f& point, const float signedDistance)
{
    const float pointDistance = SignedDistanceToPlane(plane, point);
    return point + (PlaneNormal(plane) * (signedDistance - pointDistance));
}

Camera MakeFrustumCamera()
{
...
Posef MakeFrustumCameraPose()
{
    Posef pose;
    pose.T = Vec3f(4.7f, -2.3f, 8.1f);
    pose.R = UnitQuatf(Radiansf::FromDegrees(31.0f), Vec3f::YAXIS())
        * UnitQuatf(Radiansf::FromDegrees(-17.0f), Vec3f::XAXIS());
    return pose;
}

Vec3f WorldFromCameraPoint(const Posef& cameraPose, const Vec3f& cameraPoint)
{
    return cameraPose * cameraPoint;
}

Vec2f ScreenToNdcForTest(const Vec2f& screenPos, const Viewport& viewport)
{
...
Vec3f WorldFromScreenPoint(const Camera& camera,
    const Posef& cameraPose,
    const Vec2f& screenPos,
    const float ndcDepth)
{
    const Vec2f ndc = ScreenToNdcForTest(screenPos, camera.GetViewport());
    const Mat44f invViewProj = cameraPose.ToMatrix() * camera.GetMatrix().Inverse();
...
TEST(Camera, SetPerspective_UpdatesProjectionParamsViewportAndMatrix)
{
    ...
    ExpectMatrixNear(camera.GetMatrix(), Mat44f::PerspectiveLH(fov, aspectRatio, nearClip, farClip));
}
...
TEST(Frustum, Contains_ReturnsIntersectsForSphereStraddlingFrustumPlanes)
{
    ...
    const Vec3f interiorPoint = WorldFromCameraPoint(cameraPose, Vec3f(0.31f, -0.27f, 6.4f));
...
TEST(Frustum, SelectionRectConstructor_ContainsOnlySelectedScreenRegion)
{
    const Camera camera = MakeFrustumCamera();
    const Posef cameraPose = MakeFrustumCameraPose();
    const Frustum fullFrustum(camera, cameraPose);
    const Rect selectRect({ .X = 47, .Y = 41, .Width = 37, .Height = 29 });
    const Frustum selectionFrustum(camera, cameraPose, selectRect);
