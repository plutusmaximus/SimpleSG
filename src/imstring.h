...
private:
    ...
    static block* make_block(const std::array<const char*, N>& ss,
        const std::array<size_type, N>& nn)
    {
...
public:
    ...
    constexpr imstring() noexcept
        : m_blk(nullptr)
    {
    }

    imstring(const char* s)
        : m_blk(make_block(std::array<const char*, 1>{ s },
              std::array<size_type, 1>{ s ? std::char_traits<char>::length(s) : 0 }))
    {
    }
    ...
    {
        if(n && !s)
            throw std::invalid_argument("imstring");
    }

    imstring(std::string_view sv)
        : m_blk(make_block(std::array<const char*, 1>{ sv.data() },
              std::array<size_type, 1>{ sv.size() }))
    {
    }

    imstring(const std::string& s)
        : m_blk(make_block(std::array<const char*, 1>{ s.data() },
              std::array<size_type, 1>{ s.size() }))
    {
    }

    imstring(const imstring& o) noexcept
        : m_blk(o.m_blk)
    {
        retain(m_blk);
    }

    imstring(imstring&& o) noexcept
        : m_blk(o.m_blk)
    {
        o.m_blk = nullptr;
    }
    ...
    ~imstring() { release(m_blk); }

    const char* data() const noexcept { return m_blk ? m_blk->data : ""; }

    const char* c_str() const noexcept { return data(); }

    size_type size() const noexcept { return m_blk ? m_blk->size : 0; }
    ...
    {
        if(i >= size())
            throw std::out_of_range("imstring");
        return data()[i];
    }
    ...
    bool contains(std::string_view n) const noexcept { return find(n) != npos; }
    ...
    friend std::strong_ordering operator<=>(const imstring& a, const imstring& b) noexcept
    {
        return a.view() <=> b.view();
    }
};
...
inline std::ostream&
operator<<(std::ostream& os, const imstring& s)
{
    return os << s.view();
}
...
inline imstring
operator"" _is(const char* s, std::size_t n)
{
    return imstring(s, n);
}
